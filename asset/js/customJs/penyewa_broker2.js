  ...
  function request_penyewa(){
      ...
      $(".loading").show();
      $.post('../../../proses/transaksi_broker.php', {getListBroker : input},
      function (data) {
        res = JSON.parse(data);
        if(res.length==0){
  ...
  $("#pilih-penyewa").click(function(){
    ...
    if(nama!="" && alamat!="" && jenis_kelamin!="" && email!="" && no_tlp!=""){
      ...
      $(this).text("Mendaftaran Broker...");
      $.post('../../../proses/transaksi_broker.php', {
        daftar_broker : nama, alamat : alamat, email : email,
        jenis_kelamin : jenis_kelamin, no_tlp : no_tlp
      },
      function (data) {
          response = JSON.parse(data);
          if(response.status!="gagal"){
            $("#pilih-penyewa").text("Broker berhasil didaftarkan");
            setTimeout(function(){
              $(".close").click();           
